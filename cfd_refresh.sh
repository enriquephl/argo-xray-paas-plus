...
vllink=$(echo -e '\x76\x6c\x65\x73\x73')"://"$UUID"@"$ARGO_URL":443?encryption=none&security=tls&type=ws&host="$ARGO_HOSTNAME"&path="$VLESS_WSPATH"?ed=2048#${DISPLAY_NAME}vless"
...
trlink=$(echo -e '\x74\x72\x6f\x6a\x61\x6e')"://"$UUID"@"$ARGO_URL":443?security=tls&type=ws&host="$ARGO_HOSTNAME"&path="$TROJAN_WSPATH"?ed2048#${DISPLAY_NAME}trojan"
